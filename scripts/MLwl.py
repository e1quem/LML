...
df_wl = pd.read_csv(f'out/enriched_watchlist_' + name +'.csv', sep=None, engine='python')
wl_info = df_wl[['title', 'url']].copy()
...
df = df.drop(columns=[
    ...
    'rating_5.0_count','theme_1', 'theme_2'], errors='ignore')
...
# Defining average rating per thematics, with a leave-one-out mechanism to avoid overfitting
...
# Applying encoding to watchlist
def apply_target_encoding_to_wl(train_df, wl_df, cols, target):
    melted = train_df.melt(id_vars=[target], value_vars=cols).dropna()
    melted = melted[melted['value'] != '']
    stats = melted.groupby('value')[target].agg(['sum', 'count'])
    global_mean = train_df[target].mean()
    ...
    return wl_df.apply(get_mean_wl, axis=1)
...
for cols in [
    ...
    df_wl[column_name] = apply_target_encoding_to_wl(df_train, df_wl, enc_cols, 'user_rating')
...
# Formatting our WL dataset
df_wl['is_niche'] = np.where(df_wl['total_ratings'] > 500, 0, 1)
for col in ['views', 'likes', 'fans', 'total_ratings']:
    df_wl[col] = np.log1p(df_wl[col])

df_wl['controversy'] = df_wl['rating_std_dev'] * df_wl['total_ratings']
df_wl['year'] = 2026 - df_wl['year']
df_wl['rating_skew'] = df_wl.apply(calculate_skew, axis=1)

df_wl = process_text(df_wl)


# MODEL
# Defining Catboost matrix
# We define 3 targets for our models: absolute, delta and binary (like)
...
# Respective weighting for each model: 20 when normally distributed, 40 when not enough extreme values
...
X_wl = df_wl.drop(columns=['title', 'url', 'user_rating', 'user_like'], errors='ignore')
X_wl = X_wl.reindex(columns=X_train.columns, fill_value=-1)
if cat_features:
    X_wl[cat_features] = X_wl[cat_features].fillna('None')
X_wl[text_features] = X_wl[text_features].fillna('')
X_wl = X_wl.fillna(-1)
...
# Models inference
w_p_abs = model_abs.predict(pool_wl)
w_p_delta = model_delta.predict(pool_wl) + X_wl['avg_rating'].values
w_p_like_prob = model_like.predict_proba(pool_wl)[:, 1]
wl_meta_X = np.column_stack((w_p_abs, w_p_delta, w_p_like_prob))
wl_final_ratings = meta_model.predict(wl_meta_X)

# Results
wl_info['Predicted_Rating'] = wl_final_ratings
wl_info['Like_Probability'] = w_p_like_prob
wl_info = wl_info.sort_values(by='Predicted_Rating', ascending=False)
print(wl_info[['title', 'Predicted_Rating', 'Like_Probability']].head(10))
print(wl_info[['title', 'Predicted_Rating', 'Like_Probability']].tail(10))